...
app.get('/viewEditTracker/:routes', function (req, res, next) {
...
res.locals.aid=pid[1];
res.locals.tid=pid[0];
...
app.post(/\/viewEditTracker(?:\/(\w+))?/, function (req, res, next) {
    // reuse the admin's database connection
    var db = res.locals._admin.db;
...
app.all(/\/viewEditTracker(?:\/(\w+))?/, function (req, res, next) {
...
var pass=bcrypt.hashSync(formdata.Account_Password);
   ...
   var sql = "update tracker set tracker_imei='"+formdata.tracker_imei+"',purchasing_date='"+formdata.tracker_date+"', account_id='"+formdata.account_id+"' \
where `tracker_id` = '"+formdata.tracker_id+"' ";
